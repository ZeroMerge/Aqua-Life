...
type TabId = 'live' | 'analytics' | 'thresholds';
...
export default function App() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-al-off-white text-al-near-black font-sans selection:bg-al-blue selection:text-white transition-colors duration-300">
      <header className="sticky top-0 z-50 glass-panel px-6 h-16 flex items-center justify-between shrink-0 border-b border-al-light-gray/50">
        <div className="flex items-center">
          <h1 className="text-[19px] font-semibold tracking-tight text-al-near-black">AquaLife System</h1>
        </div>

        <div className="flex items-center gap-5">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-al-light-gray/40 rounded-[6px]">
            <StatusDot status={systemStatus} />
            <span className="text-[13px] font-medium text-al-dark-gray">
          ...
          <div className="flex items-center gap-3">
            ...
            {session && (
              <button onClick={() => getSupabaseClient()?.auth.signOut()} className="text-[13px] font-medium text-al-mid-gray hover:text-[#ff3b30] transition-colors">
                Sign Out
              </button>
            )}
          </div>
      ...
      <div className="flex justify-center w-full pt-8 pb-4 px-4 sticky top-16 z-40 bg-al-off-white/80 backdrop-blur-md">
        <nav className="flex p-1 space-x-1 bg-al-light-gray/50 rounded-[8px] shadow-inner border border-black/5 relative" role="tablist">
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative px-5 py-1.5 text-[14px] font-medium rounded-[6px] transition-colors duration-200 focus-visible:outline-none flex items-center z-10 ${isActive ? 'text-al-near-black' : 'text-al-mid-gray hover:text-al-near-black'
                  }`}
              >
                {isActive && (
                  <motion.div
